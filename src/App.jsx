...
const MembersPage = ({ role }) => {
  ...
  const [form, setForm] = useState({ name:"",birthdate:"",address:"",category:"Official Member",type:"Young Adult",branch:"Main – Pinamalayan",lifegroupLeader:"" });
  ...
  const filtered = members
    .filter(m=>{
      ...
      const matchQ = m.name.toLowerCase().includes(q)||m.address.toLowerCase().includes(q)||m.branch.toLowerCase().includes(q);
  ...
  const save = () => {
    const age = calcAge(form.name);
    if(editId) {
      setMembers(members.map(m=>m.id===editId?{...m,...form,age:calcAge(form.birthdate)}:m));
    } else {
      const newM = { ...form, id:Date.now(), age:calcAge(form.birthdate), points:0, attendance:0 };
      setMembers(prev=>[...prev,newM].sort((a,b)=>a.name.localeCompare(b.name)));
    }
    setShowModal(false); setEditId(null);
    setForm({ name:"",birthdate:"",address:"",category:"Official Member",type:"Young Adult",branch:"Main – Pinamalayan",lifegroupLeader:"" });
  };

  const startEdit = m => { setForm({ name:m.name,birthdate:m.birthdate,address:m.address,category:m.category,type:m.type,branch:m.branch,lifegroupLeader:m.lifegroupLeader }); setEditId(m.id); setTab("manual"); setShowModal(true); };
  ...
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16, flexWrap:"wrap", gap:10 }}>
        ...
        <div style={{ display:"flex", gap:8 }}>
          <Btn label="Add Member" icon={Ico.plus} onClick={()=>{ setEditId(null); setTab("manual"); setShowModal(true); }} sm/>
          {role==="superadmin" && <Btn label="Upload Sheet" icon={Ico.upload} outline sm onClick={()=>{ setEditId(null); setTab("upload"); setShowModal(true); }}/>}
        </div>
      </div>

      {/* Filters */}
      <div style={{ display:"flex", gap:8, marginBottom:14, flexWrap:"wrap" }}>
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search name, address, branch…"
          style={{ flex:1, minWidth:180, padding:"9px 14px", borderRadius:R.full, border:`1.5px solid ${C.cloud}`, fontSize:13, outline:"none", color:C.ink }}/>
      </div>
      <div style={{ display:"flex", gap:6, marginBottom:16, flexWrap:"wrap" }}>
        {["All",...CATEGORIES].map(t=><Pill key={t} label={t} active={filterCat===t} onClick={()=>setFilterCat(t)} color={t==="All"?C.blue:catColor(t)}/>)}
      </div>
      <div style={{ display:"flex", gap:6, marginBottom:18, flexWrap:"wrap" }}>
        {["All",...MEMBER_TYPES].map(t=><Pill key={t} label={t} active={filterType===t} onClick={()=>setFilterType(t)} color={C.slate}/>)}
      </div>

      {/* Count */}
      <div style={{ fontSize:12, color:C.mist, marginBottom:12 }}>{filtered.length} member{filtered.length!==1?"s":""} · sorted alphabetically</div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:`repeat(auto-fill,minmax(${mob?280:300}px,1fr))`, gap:14 }}>
            ...
            <Card key={m.id} hoverable>
              <div style={{ display:"flex", alignItems:"flex-start", gap:12, marginBottom:12 }}>
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ fontWeight:700, fontSize:14, color:C.ink, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{m.name}</div>
                  <div style={{ fontSize:11, color:C.mist }}>{m.type} · {m.branch.split("–")[0].trim()}</div>
                </div>
                <Badge label={m.category} color={catColor(m.category)}/>
              </div>

              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:6, fontSize:12, color:C.slate, marginBottom:12 }}>
                <div>🎂 {m.birthdate||"—"}</div>
                <div>Age {m.age}</div>
                <div style={{ gridColumn:"1/-1", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>📍 {m.address}</div>
                <div style={{ gridColumn:"1/-1", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>👤 {m.lifegroupLeader}</div>
              </div>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:14 }}>
                ...
                <div style={{ flex:1 }}>
                  <div style={{ display:"flex", justifyContent:"space-between", marginBottom:3 }}>
                    ...
                    <span style={{ fontSize:11, color:C.mist }}>{m.attendance}% att.</span>
                  </div>
                  <Bar value={m.points} max={900} color={rank.color}/>
                  <div style={{ fontSize:11, color:C.mist, marginTop:3 }}>{m.points} pts</div>
                </div>
              </div>

              <div style={{ display:"flex", gap:8 }}>
                <Btn label="Edit" icon={Ico.edit} outline sm onClick={()=>startEdit(m)}/>
                <Btn label="QR" icon={Ico.qr} outline sm onClick={()=>setQrMember(m)}/>
                {role==="superadmin" && <Btn label="Impersonate" icon={Ico.eye} outline sm onClick={()=>alert(`Impersonating ${m.name}…`)}/>}
              </div>
            </Card>
          );
        })}
        {filtered.length===0 && (
          <div style={{ gridColumn:"1/-1", textAlign:"center", padding:"50px 0", color:C.mist }}>
            <Ico.users size={40} color={C.cloud}/>
            <div style={{ marginTop:10, fontSize:14 }}>No members found</div>
          </div>
        )}
      </div>

      {/* Add / Edit Modal */}
      <Modal open={showModal} onClose={()=>{ setShowModal(false); setEditId(null); }} title={editId?"Edit Member":"Add Member"} width={480}>
        {!editId && (
          <div style={{ display:"flex", gap:8, marginBottom:18 }}>
            <Pill label="Manual Entry" active={tab==="manual"} onClick={()=>setTab("manual")}/>
            <Pill label="Upload Sheet" active={tab==="upload"} onClick={()=>setTab("upload")} color={C.green}/>
          </div>
        )}

        {tab==="upload" && !editId ? (
          <div>
            <div style={{ border:`2px dashed ${C.cloud}`, borderRadius:R.xl, padding:"36px 20px", textAlign:"center", marginBottom:16 }}>
              <Ico.upload size={36} color={C.mist}/>
              <div style={{ fontWeight:600, fontSize:14, color:C.ink, marginTop:12 }}>Drop CSV / Excel here</div>
              <div style={{ fontSize:12, color:C.mist, marginTop:4 }}>Columns: Name, Birthdate, Address, Category, Type, Branch, LifegroupLeader</div>
              <Btn label="Browse File" icon={Ico.file} sm onClick={()=>alert("File picker — auto-sorts by name on import")} style={{ marginTop:14 }}/>
            </div>
            <div style={{ background:C.fog, borderRadius:R.lg, padding:"12px 14px", fontSize:12, color:C.slate }}>
              <strong style={{ color:C.ink }}>Auto-sort enabled:</strong> Members will be sorted alphabetically by surname on import. Duplicate detection by full name.
            </div>
          </div>
        ) : (
          <>
            <Inp label="Full Name" value={form.name} onChange={v=>setForm({...form,name:v})} placeholder="e.g. Maria Santos" required/>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
              <Inp label="Birthdate" type="date" value={form.birthdate} onChange={v=>setForm({...form,birthdate:v})} required/>
              <div style={{ display:"flex", flexDirection:"column", gap:5 }}>
                <label style={{ fontSize:12, fontWeight:600, color:C.slate, letterSpacing:.2 }}>Age</label>
                <div style={{ padding:"10px 14px", border:`1.5px solid ${C.fog}`, borderRadius:R.md, fontSize:14, color:C.mist, background:C.fog }}>
                  {form.birthdate ? calcAge(form.birthdate) : "—"}
                </div>
              </div>
            </div>
            <Inp label="Address" value={form.address} onChange={v=>setForm({...form,address:v})} placeholder="e.g. Sto. Tomas, Pinamalayan" required/>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
              <Inp label="Category" value={form.category} onChange={v=>setForm({...form,category:v})} options={CATEGORIES} required/>
              <Inp label="Type" value={form.type} onChange={v=>setForm({...form,type:v})} options={MEMBER_TYPES} required/>
            </div>
            <Inp label="Branch" value={form.branch} onChange={v=>setForm({...form,branch:v})} options={BRANCHES}/>
            <Inp label="Lifegroup Leader" value={form.lifegroupLeader} onChange={v=>setForm({...form,lifegroupLeader:v})} placeholder="e.g. Ptr. Rico Cruz"/>
            <div style={{ display:"flex", gap:8, marginTop:4 }}>
              <Btn label={editId?"Save Changes":"Add Member"} icon={Ico.check} onClick={save} full/>
      ...
      <Modal open={!!qrMember} onClose={()=>setQrMember(null)} title="Member QR Code" width={380}>
        {qrMember && (
          <div style={{ textAlign:"center" }}>
            ...
            <div style={{ fontSize:12, color:C.mist, marginBottom:14 }}>{qrMember.memberCode || `JIL-${String(qrMember.id).padStart(6,"0")}`}</div>
            <div style={{ display:"flex", justifyContent:"center", marginBottom:14 }}>
              <QRDisp data={memberQRData({ ...qrMember, memberCode: qrMember.memberCode || `JIL-${String(qrMember.id).padStart(6,"0")}` })} size={180}/>
            </div>
            <div style={{ display:"flex", gap:8, justifyContent:"center" }}>
              <Btn label="Download" icon={Ico.download} outline sm onClick={()=>alert("Downloading member QR PNG")}/>
              <Btn label="Print" icon={Ico.file} sm onClick={()=>alert("Printing member ID")}/>
            </div>
          </div>
        )}
      </Modal>
